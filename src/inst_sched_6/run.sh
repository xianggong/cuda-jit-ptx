...
# echo "blockSize = $blockSize"
  k0=$(nvprof ./inst_sched_6 $dataCount $scale $blockSize kernel0.ptx 2>&1 | grep '  kernel' | awk '{print $2}' 2>&1)
  k1=$(nvprof ./inst_sched_6 $dataCount $scale $blockSize kernel1.ptx 2>&1 | grep '  kernel' | awk '{print $2}' 2>&1)
  k2=$(nvprof ./inst_sched_6 $dataCount $scale $blockSize kernel2.ptx 2>&1 | grep '  kernel' | awk '{print $2}' 2>&1)
...
# echo "scale=10; $k0Value/$k2Value*100"
  ...
  echo "$dataCount $scale $k0 $k1 $k2 $speedup01% $speedup02%" >> ${fileName}.txt
  done
done
