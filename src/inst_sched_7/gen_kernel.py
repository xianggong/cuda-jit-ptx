...
class genKernel(object):
	...
	def genALU(self, kernelIdx, bufName):
		src = "  ; ALU on " + bufName + "\n"
		src += "  %k" + str(kernelIdx) + ".alu." + bufName + ".0 = fdiv float %k"
		src += str(kernelIdx) + ".val" + bufName + ", 1.0\n"
		for i in range(self.aluWindowSize):
			src += "  %k" + str(kernelIdx) +".alu." + bufName + "." + str(i + 1) 
			src += " = fdiv float %k" + str(kernelIdx) + ".alu." + bufName 
			src += "." + str(i) + ", " + str(i + 1) + ".0\n"
		src += "  %k" + str(kernelIdx) + ".alu." + bufName + ".last = fdiv float %k"
		src += str(kernelIdx) + ".alu." + bufName + "." + str(self.aluWindowSize) + ", 1.0\n"
		src += "\n"
		return src
	...
	def dump(self):
		kernelFile = open(self.kernelName + ".ll", "w+")
		kernelFile.write(self.genIR())
		


if __name__ == "__main__":	
	aluWindowSize = int(raw_input(''))
	memDistance = int(raw_input(''))
